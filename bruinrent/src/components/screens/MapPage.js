...
const MapPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchListings = async () => {
            ...
            try {
                ...
                const sortedListingsValidAddresses = sortedListings.filter(
                    (listing) =>
                ...
                console.log(
                    `Listings set to: ${JSON.stringify(sortedListings)}`
                );
            } catch (error) {
    ...
    const handleAdminButton = async () => {
        ...
        console.log(
            "Combined Data (TO WRITE): " + JSON.stringify(combinedData)
        );
    ...
    const handleProcessClick = async () => {
        ...
        reader.onload = (event) => {
            try {
                const cleanedJsonString = cleanInvalidCharacters(
                    event.target.result
                );
